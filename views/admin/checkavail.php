...
<div class="container" id="up">
  <p style="color: royalblue;">Check Availability</p><hr>
  <div class="row">
    ...
    <div class="col-lg-4 col-sm-12 col-md-12">
      <div class="form-group">
        ...
        <input type="text"  class="form-control" name="prodcatname"  onchange="checkforactive(this.name)" placeholder="Select Any" list="prodcatname">
...
<div class="row">
	<div class="col-lg-4 col-sm-12 col-md-12">
		<div class="form-group">
			...
			<input type="text"  value="" name="status"  class="form-control" onchange="checkforactive(this.name)" placeholder="Select Any" list="status">
		</div>
	</div>
	<div class="col-lg-4 col-sm-12 col-md-12">
			<div class="form-group">
				<label for="status">Serial:<sup>*</sup>:</label>
				<input type="text"  value="" name="serial2"  class="form-control" onchange="checkforactive(this.name)" placeholder="Select Any" list="serial">
				<datalist id="status">
				<option  value="none"></option>
			...
			<option  value="none"></option>
			<?php
			$result=$conn->query("select serial from promaster");
			while($row=$result->fetch_assoc())
...
window.onload=function() {
	if((sessionStorage.getItem("value")!=null)){
		v = sessionStorage.getItem("value");
		document.getElementsByName('status')[0].value = v;
		checkforactive(v);
...
function filter()
{
	...
	serial=document.getElementsByName("serial2")[0].value;
	...
	if(status.length==0||status=="none")
		...
		filoption+="1";
	if(serial.length==0||serial=="none")
		filoption+="0";
	else
		filoption+="1"
	...
	try
	{
		...
		req.send("o="+o+"&filoption="+filoption+"&cuscode="+cuscode+"&prodcatname="+prodcatname+"&status="+status+"&serial="+serial);
	}
	catch(e)
	{
		alert(e);
...
{	o="1";
	...
	document.getElementsByName("serial2")[0].value="";
	var req=new XMLHttpRequest();
	if(cuscode!="none"){try
	{
		req.onreadystatechange=function()
